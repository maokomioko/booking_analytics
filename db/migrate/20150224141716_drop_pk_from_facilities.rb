class DropPkFromFacilities < ActiveRecord::Migration
  def up
    execute %q{ALTER TABLE room_facilities DROP CONSTRAINT room_facilities_pkey}
    add_index :room_facilities, :id, unique: true
    change_column_default :room_facilities, :id, nil

    execute %q{ALTER TABLE hotel_facilities DROP CONSTRAINT hotel_facilities_pkey}
  ...
  def down
    ...
    execute %q{ALTER TABLE room_facilities ADD PRIMARY KEY (id)}
    execute %q{
    ...
    execute %q{ALTER TABLE hotel_facilities ADD PRIMARY KEY (id)}
    execute %q{
